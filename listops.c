...
//linear search through linked list for matching strs, then print and add to output file
void
list_str_search(char target_str[], list_t *list, FILE* output_file) {
	...
	while(curr) {
		...
		//match found
		if (strcmp(curr->address, target_str) == 0) {
			
			matches += 1;

			fprintf(output_file, "--> footpath_id: %d || address: %s || clue_sa: %s || asset_type: %s ||" 
			"deltaz: %.2lf || distance: %.2lf || grade1in: %.1lf || mcc_id: %d || mccid_int: %d || rlmax:"
			" %.2lf || rlmin: %.2lf || segside: %s || statusid: %d || streetid: %d || street_group: %d ||"
			" start_lat: %lf || start_lon: %lf || end_lat: %lf || end_lon: %lf ||\n", 
			curr->footpath_id, curr->address, curr->clue_sa, curr->asset_type, curr->deltaz, 
			curr->distance, curr->grade1in, curr->mcc_id, curr->mccid_int, curr->rlmax, curr->rlmin, 
			curr->segside, curr->statusid, curr->streetid, curr->street_group, curr->start_lat, 
			curr->start_lon, curr->end_lat, curr->end_lon);
